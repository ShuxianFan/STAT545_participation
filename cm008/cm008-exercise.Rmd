...
```{r}
gapminder%>%
  mutate(gdp = gdpPercap*pop, gapBill = round(gdp/10^9,2))
```
...
```{r}
gapminder%>%
  transmute(gdp = gdpPercap*pop, gapBill = round(gdp/10^9,2))
```
...
# mutate can overwrite the existing column
...
```{r}
gapminder%>%
  summarize(mu = mean(lifeExp), median = median(lifeExp))
```
...
```{r}
#ggplot does not recognize the group made 
gapminder%>%
  group_by(country)%>%
...
cda = gapminder%>%
  ...
  `[[`("gdp")
# `[[`is a function to extract the vector

my_df = gapminder%>%
  mutate(cda_gdp = rep(cda, nlevels(country)),
         gdp = gdpPercap*pop,
         gdpPercapRel = gdp/cda_gdp)
